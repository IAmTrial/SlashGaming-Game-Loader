...
Glide3xVersion D2Version::getGlide3xVersion() {
    static Glide3xVersion glide3xVersion =
        getGlide3xVersion(determineVersionString(L"glide3x.dll"));
...
Glide3xVersion D2Version::getGlide3xVersion(std::string_view versionString) {
    ...
    return (versionIt != stringToGlide3xVersion.cend())
        ? versionIt->second
...
std::string D2Version::determineVersionString(std::wstring_view filePath) {
    ...
    std::unique_ptr<WCHAR[]> versionData(new WCHAR[versionSize]);
    if (!GetFileVersionInfoW(filePath.data(), versionHandle, versionSize, versionData.get())) {
        return "";
    }
    ...
    stringStream << ((versionInfo->dwFileVersionMS >> 16) & 0xffff) << ".";
    stringStream << ((versionInfo->dwFileVersionMS >> 0) & 0xffff) << ".";
    stringStream << ((versionInfo->dwFileVersionLS >> 16) & 0xffff) << ".";
    stringStream << ((versionInfo->dwFileVersionLS >> 0) & 0xffff);
